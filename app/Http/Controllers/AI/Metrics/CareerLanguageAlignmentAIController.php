...
private function buildQuery($careerIds = [], $groupBy = 'week')
{
    ...
    ORDER BY fecha_inicio ASC, alineacion_lenguajes DESC;
    ";
    ...
    public function getData(Request $request)
    {
        try {
            ...
            $params = [
                $startDate, $endDate, // metricas_filtradas
                $startDate, $endDate, // metricas_previas
                $startDate, $endDate, // promedio_global
    ...
    $startDate, $endDate
            ...
            return response()->json([
                ...
                'end_date' => $endDate,
                'group_by' => $groupBy,
                'avg_alignment' => round($collection->avg('alineacion_lenguajes'), 2),
                'total_careers' => $collection->pluck('id_carrera')->unique()->count(),
                'trend_data' => $trendData,
                'results' => $results,
                'message' => "📊 Datos agrupados por $groupBy (modelo 4D optimizado)."
            ]);
        } catch (\Throwable $e) {
            Log::error('❌ [getData] Error', ['message' => $e->getMessage()]);
...
public function export(Request $request)
{
    try {
        ...
        // ✅ Generamos el SQL adaptado (por carrera)
        $sql = $this->buildQuery($careerIds, $groupBy);

        // ✅ Solo 6 parámetros, porque el SQL usa 3 pares de fechas
        ...
        if ($format === 'excel') {
            $exportData = $data->map(fn($r) => [
                'Carrera' => $r->carrera,
                'Periodo' => $r->periodo,
                'Lenguajes Vinculados' => $r->lenguajes_vinculados ?? 0,
                'Empleos Actuales' => $r->empleos_actuales,
                'Promedio Empleos' => $r->promedio_empleos,
                'Alineación (%)' => $r->alineacion_lenguajes,
            ]);

            return Excel::download(
                new \App\Exports\ArrayExport($exportData->toArray(), [
                    'title' => 'Alineación de Carreras (Modelo 4D por Lenguaje)',
                    'created_at' => now()->format('d/m/Y H:i'),
                ]),
                "Alineacion_Carreras_{$groupBy}_" . now()->format('Ymd_His') . ".xlsx"
            );
        }

        // 🟥 PDF Export
        if ($format === 'pdf') {
            $pdf = Pdf::loadView('exports.languages_alignment_report', [
                'data' => $data,
                'groupBy' => $groupBy,
                'startDate' => $startDate,
                'endDate' => $endDate,
            ...
            return $pdf->download("Alineacion_Carreras_{$groupBy}_" . now()->format('Ymd_His') . ".pdf");
        }
