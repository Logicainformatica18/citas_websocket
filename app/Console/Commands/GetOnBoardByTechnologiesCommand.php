...
class GetOnBoardByTechnologiesCommand extends Command
{
    ...
    protected $description = '🧩 Recorre todas las tecnologías y guarda métricas de GetOnBoard.';
    ...
    public function handle()
    {
        ...
        $this->info("🧠 Iniciando scraping de GetOnBoard por tecnología ({$technologies->count()} tecnologías)...");
        ...
        foreach ($technologies as $techId => $techName) {
            ...
            for ($page = 1; $page <= $pages; $page++) {
                $url = "https://www.getonbrd.com/api/v0/search/jobs?query=" . urlencode($techName) . "&page={$page}&per_page=100";

                try {
                    $response = Http::timeout(25)->get($url);
                    if ($response->failed()) {
                        $this->warn("❌ Falló la API para {$techName} (página {$page})");
                        continue;
                    }
                    ...
                    Log::error("⚠️ Error en {$techName} (página {$page}): " . $th->getMessage());
                }
            }
