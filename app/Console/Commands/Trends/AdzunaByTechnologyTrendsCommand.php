...
class AdzunaByTechnologyTrendsCommand extends Command
{
    protected $signature = 'adzuna:trends-technologies
        {--country=us}
        {--pages=1}
        {--year=}
        {--quarter=}';

    protected $description = '📡 Rastrea ofertas laborales desde Adzuna usando TechnologyTrends (TECNOLOGÍAS) como semilla y vincula jobs reales';
    ...
    public function handle()
    {
        ...
        /* =====================================================
           1️⃣ TECHNOLOGY TRENDS (MISMA LÓGICA QUE CERTIFICACIONES)
        ===================================================== */
        $topics = TechnologyTrend::where('year', $year)
            ->where('quarter', $quarter)
            ->whereRaw("JSON_UNQUOTE(JSON_EXTRACT(raw_data, '$.intent')) = 'technology_trend'")
            ->whereNotLike('topic_category', 'Certificacion%')
            ->get();

        $this->info("📡 Tendencias tecnológicas a procesar: {$topics->count()}");
        ...
        foreach ($topics as $topic) {
            ...
            $this->warn("\n🔍 Trend tecnológico: {$topic->topic_name}");
            ...
            for ($page = 1; $page <= $pages; $page++) {
                ...
                /* =====================================================
                   2️⃣ KEYWORDS (IDÉNTICO AL ORIGINAL)
                ===================================================== */
                $keywords = [];
                ...
                foreach ($keywords as $keyword) {
                    ...
                    try {
                        ...
                        Log::error(
                            "TrendTech {$topic->topic_name} / {$keyword}: {$e->getMessage()}"
                        );
                    }
                }
            }
            ...
            /* =====================================================
               6️⃣ AGREGADO FINAL REAL (IDÉNTICO)
            ===================================================== */
        ...
        $this->info("\n🎯 Scan de Technology Trends completado");
