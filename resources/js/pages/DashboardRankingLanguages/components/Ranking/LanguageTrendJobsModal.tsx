...
interface Props {
  open: boolean;
  onClose: () => void;
  trendId: number | null;   // 👈 id del technology_trend
  title?: string;           // 👈 nombre del lenguaje / tendencia
}

export default function LanguageTrendJobsModal({
  open,
  onClose,
  trendId,
  title,
  ...
  const fetchJobs = async (pageNumber: number) => {
    ...
    try {
      const res = await axios.get(
        `/dashboard/ranking/languages/trend/${trendId}/jobs`,
        {
          params: {
            page: pageNumber,
            per_page: PER_PAGE,
      ...
      console.error("❌ Error cargando jobs de lenguaje en tendencia", error);
      setJobs([]);
      setTotal(0);
    } finally {
      setLoading(false);
      ...
      >
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
              {title
                ...
                : "Ofertas laborales asociadas al lenguaje en tendencia"}
            </h2>
          </div>
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto">
          ...
          {!loading && jobs.length === 0 && (
            <div className="text-center text-slate-500">
              No se encontraron ofertas para este lenguaje en tendencia.
            </div>
          )}
