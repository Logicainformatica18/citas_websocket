...
export default function ProfileAgeCard() {
  ...
  useEffect(() => {
    axios.get("/api/ai/stackoverflow/profile/age/metadata").then((res) => setMetadata(res.data));
  }, []);

  useEffect(() => {
    axios
      .get("/api/ai/stackoverflow/profile/age", { params: filters })
      .then((res) => {
        const a = res.data.age_groups || {};
        setData(Object.entries(a).map(([name, value]) => ({ name, value })));
      });
  }, [filters]);
    ...
    <Card className="bg-[#161616] border border-gray-700 text-white relative">
      <CardContent className="p-4">
        <div className="flex justify-between items-center mb-2">
          <h3 className="font-semibold text-blue-400 text-sm">Grupos etarios</h3>
        ...
        {data.length ? (
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={data} layout="vertical" margin={{ top: 10, right: 20, bottom: 10, left: 100 }}>
              ...
              <YAxis dataKey="name" type="category" tick={{ fill: "#aaa", fontSize: 10 }} />
              <Tooltip />
