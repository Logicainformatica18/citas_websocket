...
export function useAiChatLogic() {
    ...
    const handleSend = async (customText?: string, trainingId?: number) => {
        ...
        // 🛑 FIX — Si la sugerencia trae trainingId: usar "chat" sí o sí
        if (trainingId) {
            console.log("→ Ejecutando sugerencia con trainingId:", trainingId);
            setMode("chat");
        }
        ...
        try {
            ...
            if (mode === "train") {
                ...
                // 🟩 Si la sugerencia ejecuta un entrenamiento guardado
                if (trainingId && data.training_executed) {
                    const newMessages: Message[] = [
                        { from: "ai", text: `📘 Ejecutando entrenamiento guardado #${trainingId}` },
                        { from: "ai", text: data.message || "Consulta procesada correctamente." }
                    ];
                    ...
                    if (data.voice_url) {
                        ...
                        if (voiceEnabled) playAudio(data.voice_url);
                    }

                    setMessages(prev => [...prev, ...newMessages]);
                    return; // ⛔ necesario para que NO siga a los demás bloques
                }
...
if (data.topic && data.result) {
    ...
    const newMessages: Message[] = [
        ...
        { from: "ai", text: data.message || "🔄 Datos actualizados." },
    ];
    ...
    if (data.excel_download_url) {
        newMessages.push({
            from: "ai",
            text: `📊 [Descargar Excel con los datos](${data.excel_download_url})`,
        });
    }
  ...
  if (data.chart_builder_enabled && data.training_id) {
    ...
    newMessages.push({
        from: "ai",
        text: "📊 ¿Quieres generar un gráfico con estos datos?",
        chartSelector: {
