// ✅ InternalStateModal.tsx traducido al español
...
export default function InternalStateModal({
  open,
  onClose,
  onSaved,
  itemToEdit,
  ...
  const handleSubmit = async () => {
    try {
      setSaving(true);
      const url = itemToEdit ? `/internal-states/${itemToEdit.id}` : '/internal-states';
      const method = itemToEdit ? 'put' : 'post';
      const response = await axios[method](url, formData);

      toast.success(itemToEdit ? 'Estado interno actualizado ✅' : 'Estado interno creado ✅');
      onSaved(response.data.internalState);
      onClose();
    } catch (err) {
      console.error(err);
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => !val && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{itemToEdit ? 'Editar Estado Interno' : 'Nuevo Estado Interno'}</DialogTitle>
        </DialogHeader>
