// ✅ InternalStates index.tsx con tabla más estética y responsiva
...
import InternalStateModal from './modal';

const breadcrumbs: BreadcrumbItem[] = [
  { title: 'Estados Internos', href: '/internal-states' },
];
...
export default function InternalStates() {
  const { internalStates: initialPagination } = usePage<{ internalStates: Pagination<InternalState> }>().props;
  const [items, setItems] = useState(initialPagination.data);
  const [pagination, setPagination] = useState(initialPagination);
  const [showModal, setShowModal] = useState(false);
  const [editItem, setEditItem] = useState<InternalState | null>(null);
  const [selectedIds, setSelectedIds] = useState<number[]>([]);

  const handleSaved = (saved: InternalState) => {
    setItems((prev) => {
  ...
  const fetchItem = async (id: number) => {
    const res = await axios.get(`/internal-states/${id}`);
    setEditItem(res.data.internalState);
    setShowModal(true);
  };
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <div className="p-8">
        <h1 className="text-2xl font-bold mb-4">Estados Internos</h1>
            ...
            <button
              onClick={async () => {
                if (confirm(`¿Eliminar ${selectedIds.length} elemento(s)?`)) {
                  await axios.post('/internal-states/bulk-delete', { ids: selectedIds });
                  ...
                  <td className="px-4 py-2 space-x-2">
                    ...
                    <button onClick={async () => {
                      if (confirm('¿Eliminar este elemento?')) {
                        await axios.delete(`/internal-states/${item.id}`);
        ...
        <div className="flex justify-center mt-6 gap-2">
          {[...Array(pagination.last_page)].map((_, index) => {
            ...
            return (
              <button
                key={page}
                onClick={() => fetchPage(`/internal-states/fetch?page=${page}`)}
