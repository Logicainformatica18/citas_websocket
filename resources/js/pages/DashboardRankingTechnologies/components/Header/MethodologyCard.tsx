...
export default function MethodologyCard({
  ...
  const trends = Number(trendsWeight) || 100 - labor;
  ...
  return (
    <div className="border rounded-xl p-4 text-sm bg-[#F5FCFE] dark:bg-[#0F2A3A] dark:border-[#1E3A4A]">
      ...
      <ul className="space-y-1 text-slate-700 dark:text-slate-300">
        <li>
          • <strong>{labor}%</strong> Demanda laboral de tecnologías
        </li>
        <li>
          • <strong>{trends}%</strong> Presencia en reportes de tendencias tecnológicas
        </li>
      </ul>

      <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
        El score final se calcula ponderando la demanda laboral y la
        relevancia en reportes de tendencias, según la metodología seleccionada.
      </p>
